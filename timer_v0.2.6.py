...
class TerminalTimer:
    ...
    def log_timer_complete(self, log_number, task_success=None, reward_executed=None, additional_time=None, needs_additional_time=None):
        """타이머 완료 로그를 업데이트합니다."""
        logs = self._load_logs()
        for log in logs:
            if log.get('number') == log_number:
                ...
                if additional_time is not None:
                    log['additional_time_seconds'] = additional_time
                    log['additional_time_formatted'] = self.format_time(additional_time)
                if needs_additional_time is not None:
                    log['needs_additional_time'] = bool(needs_additional_time)
                break
        self._save_logs(logs)
    ...
    def display_timer(self, remaining_seconds, total_seconds, task_description=""):
        ...
        output_lines.extend([
            "=" * 60,
            "Ctrl+C: 타이머 중지 | P: 일시정지/재개 | Y: 조기 완료"
        ])
    ...
    def countdown(self, total_seconds, task_description="", log_number=None, reward_info=None):
        ...
        early_completion = False  # 조기 완료 플래그
        ...
        try:
            while remaining_seconds > 0 and self.is_running:
                ...
                if ch == 'p':
                    ...
                    remaining_seconds = 0  # 루프 종료를 위해 0으로 설정
                    self.is_running = False
                    break
            ...
            if self.is_running:
                ...
                print("=" * 60)
                if early_completion:
                    print(f"남은 시간이 있었지만 {self.COLORS['GREEN']}{self.COLORS['BOLD']}조기 완료{self.COLORS['RESET']}로 처리되었습니다.")
                else:
                ...
                additional_time_seconds = None
                needs_additional_time = None
                
                # 조기 완료인 경우 자동으로 Y 처리
                if early_completion:
                    task_success = True
                    print(f"\n{self.COLORS['GREEN']}{self.COLORS['BOLD']}✓ 업무를 성공적으로 완료한 것으로 처리되었습니다.{self.COLORS['RESET']}")
                else:
                    try:
                        while True:
                ...
                # n 입력 시 추가 작업 시간 물어보기
                if not task_success:
                    try:
                        while True:
                            additional_input = input("\n추가작업을 진행할까요? (5분 희망 시 '5'라고 입력 or N): ").strip()
                            if additional_input.upper() == 'N':
                                # 추가 시간 필요없음으로 처리
                                needs_additional_time = False
                                # 미완수 업무로 저장 (다음 타이머에서 자동으로 사용)
                                self.pending_task = task_description
                                print(f"\n{self.COLORS['YELLOW']}추가 시간이 필요없음으로 처리되었습니다.{self.COLORS['RESET']}")
                                print(f"다음 타이머 시작 시 '{task_description}' 업무가 자동으로 설정됩니다.")
                                break
                            elif additional_input.isdigit():
                                # 숫자 입력 시 해당 시간(분)만큼 추가 타이머 실행
                                additional_minutes = int(additional_input)
                                additional_time_seconds = additional_minutes * 60
                                needs_additional_time = True
                                ...
                                # 추가 타이머 실행
                                print("\n" + "=" * 60)
                                print(f"⏰ 추가 작업 타이머를 시작합니다.")
                                print(f"추가 시간: {self.COLORS['GREEN']}{self.COLORS['BOLD']}{self.format_time(additional_time_seconds)}{self.COLORS['RESET']}")
                                ...
                                # 추가 타이머 실행 (같은 업무 내용으로)
                                additional_log_number = self.log_timer_start(additional_time_seconds, task_description, None)
                                self.countdown(additional_time_seconds, task_description, additional_log_number, None)
                                
                                # 추가 타이머 완료 후 리마인더 재시작
                                self.start_reminder_thread()
                                
                                # 추가 타이머 완료 후 다시 성공 여부 확인
                                try:
                                    while True:
                                        final_answer = input("\n이제 업무를 성공적으로 완료했나요? (y/n): ").strip().lower()
                                        if final_answer in ('y', 'n'):
                                            task_success = (final_answer == 'y')
                                            break
                                        print("y 또는 n만 입력해주세요!")
                                except KeyboardInterrupt:
                                    task_success = False
                                ...
                                # 추가 타이머도 실패한 경우 미완수 업무로 저장
                                if not task_success:
                                    self.pending_task = task_description
                                
                                break
                            else:
                                print("숫자(분) 또는 N을 입력해주세요!")
                    except KeyboardInterrupt:
                        needs_additional_time = False
                        self.pending_task = task_description
                        print("\n입력이 취소되어 미완수로 처리합니다.")
                ...
                if log_number:
                    self.log_timer_complete(log_number, task_success=task_success, reward_executed=reward_executed, 
                                          additional_time=additional_time_seconds, needs_additional_time=needs_additional_time)
    ...
    def get_user_input(self):
        ...
        if self.pending_task:
            print(f"📋 미완수 업무가 있습니다: {self.COLORS['YELLOW']}{self.COLORS['BOLD']}{self.pending_task}{self.COLORS['RESET']}")
            print("=" * 60)
            print("이 업무를 계속 진행하시겠습니까?")
            print("  Enter: 미완수 업무 사용")
            print("  다른 입력: 새 업무 입력")
            print("=" * 60)
            ...
            try:
                user_choice = input("\n선택: ").strip()
                if not user_choice:
                    # Enter만 누른 경우 미완수 업무 사용
                    task_description = self.pending_task
                    self.current_task = task_description
                    self.pending_task = None  # 사용했으므로 초기화
                    print(f"\n{self.COLORS['GREEN']}미완수 업무 '{task_description}'를 사용합니다.{self.COLORS['RESET']}")
                else:
                    # 새 업무 입력 받기
                    print("\n" + "=" * 60)
